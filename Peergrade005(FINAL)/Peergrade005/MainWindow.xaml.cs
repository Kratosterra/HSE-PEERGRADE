...
namespace Peergrade005
{
    ...
    public partial class MainWindow : Window
    {
        ...
        private void SetNewEnvironment()
        {
            ...
            {
                MessageBox.Show($"Произошла критическая ошибка при установке окружения!", "Ошибка",
        ...
        private void FillExampleColor((int, int, int) RGB, bool isThisStartColor)
        {
            ...
            {
                MessageBox.Show($"Произошла ошибка при применении цветовой гаммы!", "Ошибка",
                    MessageBoxButton.OK, MessageBoxImage.Error);
            }
        }
        ...
        private void SetNewViewMod()
        {
            ...
            {
                ScrollViewer.ScrollToHorizontalOffset(MainWindowKernel.MaxWidth / 2.6);
                ScrollViewer.ScrollToVerticalOffset(MainWindowKernel.MaxHeight / 2.9);
            }
        }
        ...
        private void TreeButton_Click(object sender, RoutedEventArgs e)
        {
            ...
            {
                MessageBox.Show($"Произошла ошибка при смене типа фрактала!", "Ошибка",
                    MessageBoxButton.OK, MessageBoxImage.Error);
            }
            finally
            {
        ...
        private void RecursionDeepSlider_ValueChanged(object sender, RoutedPropertyChangedEventArgs<double> e)
        {
            ...
            if (s_fractal != null && (int)RecursionDeepSlider.Value != s_nowRecursion)
                CreateFractalButton_Click(this, new RoutedEventArgs());
        ...
        private void CreateFractalButton_Click(object sender, RoutedEventArgs e)
        {
            ...
            {
                MessageBox.Show($"Использовано слишком много памяти, измените входные данные!"
                    , "Ошибка", MessageBoxButton.OK, MessageBoxImage.Error);
            }
            catch (OverflowException)
            {
                MessageBox.Show($"Произошло переполнение, измените входные данные!",
                    "Ошибка", MessageBoxButton.OK, MessageBoxImage.Error);
            }
            catch (StackOverflowException)
            {
                MessageBox.Show($"Произошло переполнение стека, измените глубину рекурсивных вызовов!",
                    "Ошибка", MessageBoxButton.OK, MessageBoxImage.Error);
            }
            catch (Exception)
            {
